function [y,v,M] = CS4300_run_fall_model_noise(y0,v0,g,t_max,dt,sigma2)
% CS4300_run_fall_model_noise - calculates object trajectory with noise
% On input:
...
for s = 1:num_t_vals
    v(s+1) = v(s) + g*dt;
    y(s+1) = CS4300_fall_model(y(s),v(s+1),dt) + sqrt(sigma2)*randn;
    if y(s+1)<0
        y(s+1) = abs(y(s+1));
        v(s+1) = -v(s+1);
    end
